...
官方发消息接口要求机器人在线。保存并启用 QQ 机器人渠道后，主应用会按渠道里的 AppID / Secret 自动连 WebSocket。`group_openid` 可以先留空。把机器人拉进目标群后，在群设置里把机器人消息范围设为「获取群内全部消息」，然后发送 `#绑定`（多个渠道时用 `#绑定#渠道ID`）。也可以发送 `#帮助` 查看口令，或发送 `#余额`、`#公告`、`#倍率` 等立刻查询当前状态。`#倍率` 会先列出上游和对应查询命令。@ 机器人仍然有效。
...
Cron 表达式为 **6 段**（秒 分 时 日 月 周）。小时窗口按容器环境变量 `TZ` 解释（Docker Compose 默认 `Asia/Shanghai`）；镜像已包含 `tzdata`。修改 cron 后需「保存」再「应用」才会重建调度器。

默认调度：

- 余额同步：每 15 分钟（含消费采集；Sub2API 优先 `dashboard/stats`，失败回退当日 `usage` 明细汇总；消费失败不推送告警）。
- 倍率同步：每 30 分钟。
- 余额汇总推送：默认关闭；可配置例如 `0 0 9,21 * * *`，按渠道优先级汇总库内余额发一条 `balance_digest` 通知。
- 订阅用量检查：随余额同步执行，对启用订阅的 Sub2API 渠道自动采集用量并触发低余量/到期告警。
- 验证码余额刷新：随调度自动刷新，也可手动刷新。
- 历史清理：每天凌晨执行。

晚间静默示例：将余额 cron 设为 `37 */15 8-22 * * *`（仅白天采集）。
