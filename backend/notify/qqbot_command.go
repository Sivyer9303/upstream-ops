...
type QQBotInbox interface {
	...
	HandleQQBotRateQuery(ctx context.Context, arg string) string
}
...
func qqBotCommandCatalog() []qqBotCommandSpec {
	return []qqBotCommandSpec{
		...
		{Kind: QQBotCmdRateQuery, Title: "倍率", Summary: "先列上游，再按命令查当前倍率", Keywords: []string{"倍率"}},
		{Kind: QQBotCmdReport, Report: QQBotReportRateChanged, Title: "倍率变化", Summary: "最近倍率变动", Keywords: []string{"倍率变化"}},
...
func ParseQQBotUserCommand(content string) QQBotUserCommand {
	raw := normalizeQQBotCommandText(content)
	if raw == "" {
	...
	if cmd, ok := parseQQBotRateQueryCommand(raw); ok {
		return cmd
	}
...
func normalizeQQBotCommandText(content string) string {
...
func isQQBotHashCommand(content string) bool {
	return strings.HasPrefix(strings.TrimSpace(content), "#")
}

func shouldHandleQQBotGroupEvent(eventType, content string) bool {
	switch eventType {
	case "GROUP_AT_MESSAGE_CREATE":
		return true
	case "GROUP_MESSAGE_CREATE":
		return isQQBotHashCommand(content)
	default:
		return false
	}
}

func parseQQBotRateQueryCommand(raw string) (QQBotUserCommand, bool) {
	if strings.HasPrefix(raw, "倍率变化") {
		return QQBotUserCommand{}, false
	}
	rest, ok := cutQQBotCommandPrefix(raw, "倍率")
	if !ok {
		rest, ok = cutQQBotCommandPrefix(strings.ToLower(raw), "rates")
	}
	if !ok {
		return QQBotUserCommand{}, false
	}
	...
	rest = strings.TrimSpace(rest)
	return QQBotUserCommand{Kind: QQBotCmdRateQuery, Arg: rest}, true
}

func cutQQBotCommandPrefix(raw, word string) (string, bool) {
	if word == "" || !strings.HasPrefix(raw, word) {
		return "", false
	}
	rest := raw[len(word):]
	if rest == "" {
...
func QQBotHelpText() string {
	...
	b.WriteString("UpstreamOps 机器人口令（群里发 #命令，不用@）：\n")
	b.WriteString("需在群设置把机器人消息范围设为「获取群内全部消息」。@我仍然有效。\n")
	for _, spec := range qqBotCommandCatalog() {
		b.WriteString("\n#")
